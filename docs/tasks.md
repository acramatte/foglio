...
- **Tests:** workspace fmt/tests/strict Clippy; frontend tests (36 passing), typecheck and build; debug/optimized native builds; Phase 1/2 CLI acceptance (12 + 4); Phase 4 native regression and Phase 5 native acceptance (9 scenarios) pass. Exact commands in Phase 5 evidence.
- **Acceptance:** V20/V21/V22 real file lifecycle, contained preview scrolling, bidirectional Ctrl+E toggling, title-derived portable filenames, existing-tag removal choices, actual keyboard source edit, supported metadata/BOM/CRLF and mixed-newline preservation, error/retry, stale/missing buffers and real WM_DELETE_WINDOW flush/block behavior. Independent-review overlap/deferred-refresh/newline regressions failed before fixes and pass afterward. Scripted native interaction replaces a separate manual physical-display smoke for this local gate; physical-display/accessibility review remains P7.
