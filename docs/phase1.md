...
notes list
notes show Architecture.md
notes tag add Architecture.md design
notes tag remove Architecture.md rust
notes tags
notes move Architecture.md Projects/Architecture.md
notes delete Projects/Architecture.md --yes
```

These are usage examples, not commands run against a personal library. `init` selects/creates the root without changing Markdown. Supported plain Markdown is immediately available without frontmatter or IDs. Non-note files, unsupported paths and invalid documents are diagnosed; scans with diagnostics exit 1 even when valid notes were listed. Inspect incomplete reports.
...
Arguments are literal safe library-relative `.md` paths, with no selector prefixes or ID lookup. Existing `id` values are ordinary user metadata and are preserved. Identical content/metadata at distinct paths remains independently accessible and mutable. Unrelated invalid documents do not block revision-guarded operations on healthy paths.
...
`Library::open(root, state_directory, create)` creates an isolated core handle; CLI and future desktop use `Library::resolve`/`Library::init` for shared selection. The root is immutable through the handle. `scan()` returns sorted entries, diagnostics, and `incomplete`; `Report::summaries` produces lightweight path/title/tag summaries.

`create`, `get`, `update`, `move_note`, `delete`, and `tag` operate on real files. `tag(..., true)` adds; `tag(..., false)` removes, using exact case-sensitive equality and first-occurrence order. Body update is a core operation, not a Phase 1 CLI command. Mutations consume the opaque `Revision` returned with the document. `LibraryRelativePath` validates filesystem inputs; full-byte hashes protect revisions, not document identity. `ErrorCode` distinguishes domain failures without parsing prose.

Updates/moves/deletes/tags return `Commit { file_committed, durability_confirmed, path, revision }`. Paths in commits are absolute filesystem paths; entry/summary paths are library-relative. Creation returns its committed document. If creation/root selection committed but durability is uncertain, the error has code `committed` and a populated `commit` field. Do not blindly retry a committed mutation. No index outcome exists before Phase 2.
...
- `list`: result is an array of summaries (`path`, `title`, `tags`).
- `show`/`new`: result is the document entry, including complete `source`, `body`, `revision`, metadata.
- `tags`: sorted, deduplicated tag strings from supported entries.
...
| 3 | Note not found |
| 4 | Stale revision, occupied destination, or busy library |
...
Limits are 16 MiB per complete note (including generated metadata), 256 KiB frontmatter, indentation at most 64 bytes and container nesting at most 32. Invalid UTF-8, wrong-type tags, duplicate nested/quoted-equivalent keys and unsupported/resource-limited YAML are rejected before writes. Every candidate mutation is reparsed before replacement.
...
Create and move require Linux `renameat2(RENAME_NOREPLACE)` through safe `rustix` APIs, with no copy/delete or link/unlink fallback. Unsupported primitives and cross-device moves fail. Move preserves the existing inode/content. Delete unlinks only one guarded note; empty directories remain. A killed staged writer leaves the old note complete and can leave a complete non-note staging file. Scans ignore identifiable staging files; no automatic stale-temp cleanup deletes user files.

Symlinks at root, ancestor or leaf are rejected; leaf reads use `O_NOFOLLOW | O_NONBLOCK`. Hard-linked files can be read, but mutations refuse them. Ordinary Unix mode bits, owner and group are preserved on replacement. Attributes in Apple's `com.apple.*` namespace are managed by macOS itself (Finder tags, `com.apple.provenance`, TCC bookkeeping such as `com.apple.macl`) and are not POSIX access control, so replacement carries them over to the published inode and refuses the write if a copy fails. If staging would change owner/group, or either inode has unmanaged ACLs/xattrs/special mode bits, replacement is refused before publishing data. This intentionally excludes ACL-/security-label-bearing setups rather than weakening their access controls. Supported read-only Markdown remains readable without frontmatter.
...
The original Phase 1 counts above are historical. Current path-identity regression evidence is recorded in [path-identity.md](path-identity.md).

| Gate | Evidence |
|---|---|
| V01 | fmt/Clippy/build/tests; help/version/usage subprocesses preserve isolated state |
| V02, V04 | repeat init, hidden discovery, byte-preserving selection, read-only imports, invalid input diagnostics |
| V03 | parser tables and repository fixture, BOM/CRLF, aliases/limits, nested duplicate keys, real H1 |
| V05 | identical content/metadata remains independent by path; literal path operations |
| V06 | complete CLI/core lifecycle, body updates, tag idempotence, collision refusal, unchanged bytes on move |
...
Source §43's five checks are automated: ordinary generated Markdown; non-mutating imported Markdown discovery; unchanged bytes on move; unrelated YAML surviving edits; and authoritative notes surviving app-state deletion. Phases 2–4 were subsequently implemented; desktop editing/v1 release gates remain open.
