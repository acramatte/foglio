...
Status: V01–V10 Phase 1 checks and V11–V15 Phase 2 checks (V14 status portion only), plus V25 initial baseline, pass locally on Linux x86_64 with Rust 1.93.1; see [Phase 1](../phase1.md#verification-evidence) and [Phase 2 evidence/limitations](../phase2.md). V16/V17 and the V24 development simulation now pass: [Phase 3 evidence](../phase3.md). Hosted CI currently covers Phase 0 only. V18/V19 Phase 4 pass in actual Tauri/WebKit under Xvfb; see [desktop evidence and limits](../phase4.md). V20–V22 Phase 5 source editing/autosave/lifecycle pass locally, including actual WebKit keyboard edits and native-close protection; see [Phase 5 evidence](../phase5.md). V23 and the V24 Phase 6 filesystem simulation pass in native debug/optimized WebKit runs; see [Phase 6 evidence and limits](../phase6.md). V14 doctor and V26 local keyboard/labels plus staged Debian payload checks now pass; V25 final measurements are available without approved budgets. Actual cross-device transfer, fresh-OS installation, physical/assistive-technology review, physical-cold storage and large-corpus UI qualification remain blocked/unrun; Phase 7/v1 is not complete. [Phase 7 evidence](../phase7.md).
...
| V13 | Search title/body/path/tags, phrase/prefix, punctuation/Unicode, exact case-preserving tag filters, folder `foo` versus `foobar`, ranked ties, empty/malformed/SQL-like queries, safe snippets; smart prefix expansion finds `memo` in a `Memory` title, ranks complete words before expansions and titles before bodies, deduplicates across tiers and still rejects interior substrings | Search/CLI, P2; desktop/native smart search |
